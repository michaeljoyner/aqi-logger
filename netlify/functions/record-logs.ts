...
const aqiToken = process.env.AQI_TOKEN;
  ...
  cities.forEach(async (city) => {
    try {
      const resp = await fetch(`https://api.waqi.info/feed/geo:${city.lat};${city.lng}/?token=${aqiToken}`);
      const respData = await resp.json();
      await client.execute({
        sql: "INSERT INTO records (city_id, hour, value) VALUES (?, ?, ?)",
        args: [city.id, new Date(respData.data.time.s).getHours(), respData.data.aqi]
      });
    } catch(err) {}
  });
}
